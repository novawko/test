...
charadex.initialize.page = async (dataArr, config, dataCallback, listCallback, customPageUrl = false) => {
  ...
  for (let entry of charadexData) {
    ...
    if (entry.tags) entry.fancytags = `<a href="${charadex.url.addUrlParameters(charadex.url.getPageUrl(charadex.page.imageGallery.sitePage), {tags: tag})}">#${tag.trim()}</a>`; // Make the tags if any
  }
